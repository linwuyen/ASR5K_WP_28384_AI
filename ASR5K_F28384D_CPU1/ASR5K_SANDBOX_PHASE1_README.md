...
| CC sink | cc_sandbox | DEBUG＋CV 導出測試 pattern | `g_sCcSandbox.u16Sink`：1=DACB、2=MCBSP stub |
...
AM3352 注入示範（debugger）：`g_sAm3352Sandbox.u32Param=50000` →
`u16CmdId=3` → `u16Pending=1` ⇒ DDS 變 500Hz（走 AM3352→dispatcher→DDS
全鏈，`u32ForwardCount` 與 `u32CmdDispatch` 各 +1）。
...
| 並聯訊息同步（FSI 14筆×16bit/2.24µs） | fsi_sandbox | **產品 14-word frame**：seq＋三相設定＋第2/3台 V/I＋2 組自訂 cmd/data＋share%＋checksum；CV 樣本自動入 frame、share% 由 `FsiSandbox_GetSharePct()` 供 CC 迴路 |
